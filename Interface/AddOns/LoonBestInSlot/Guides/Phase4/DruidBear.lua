local spec = LoonBestInSlot:RegisterSpec("Druid", "Bear", "4")
LoonBestInSlot:AddItem(spec, "32235", "Head", "Cursed Vision of Sargeras", "BIS")
LoonBestInSlot:AddItem(spec, "33672", "Head", "Vengeful Gladiator's Dragonhide Helm", "BIS Mit")
LoonBestInSlot:AddItem(spec, "31039", "Head", "Thunderheart Cover", "Alt")
LoonBestInSlot:AddItem(spec, "31096", "Head", "Helm of the Forgotten Vanquisher", "Alt")
LoonBestInSlot:AddItem(spec, "33479", "Head", "Grimgrin Faceguard", "Alt")

LoonBestInSlot:AddItem(spec, "31048", "Shoulders", "Thunderheart Pauldrons", "BIS")
LoonBestInSlot:AddItem(spec, "31102", "Shoulders", "Pauldrons of the Forgotten Vanquisher", "BIS")

LoonBestInSlot:AddItem(spec, "33593", "Back", "Slikk's Cloak of Placation", "BIS")
LoonBestInSlot:AddItem(spec, "33484", "Back", "Dory's Embrace", "BIS Thrt")
LoonBestInSlot:AddItem(spec, "28660", "Back", "Gilded Thorium Cloak", "Alt")
LoonBestInSlot:AddItem(spec, "29994", "Back", "Thalassian Wildercloak", "Alt")

LoonBestInSlot:AddItem(spec, "31042", "Chest", "Thunderheart Chestguard", "BIS")
LoonBestInSlot:AddItem(spec, "31090", "Chest", "Chestguard of the Forgotten Vanquisher", "BIS")

LoonBestInSlot:AddItem(spec, "32324", "Bracers", "Insidious Bands", "BIS")
LoonBestInSlot:AddItem(spec, "33881", "Bracers", "Vindicator's Dragonhide Bracers", "BIS Mit")
LoonBestInSlot:AddItem(spec, "33580", "Bracers", "Band of the Swift Paw", "Alt")

LoonBestInSlot:AddItem(spec, "31034", "Gloves", "Thunderheart Gauntlets", "BIS")
LoonBestInSlot:AddItem(spec, "31093", "Gloves", "Gloves of the Forgotten Vanquisher", "BIS")

LoonBestInSlot:AddItem(spec, "30106", "Belt", "Belt of One-Hundred Deaths", "BIS")
LoonBestInSlot:AddItem(spec, "33583", "Belt", "Waistguard of the Great Beast", "Alt")

LoonBestInSlot:AddItem(spec, "31044", "Legs", "Thunderheart Leggings", "BIS")
LoonBestInSlot:AddItem(spec, "31099", "Legs", "Leggings of the Forgotten Vanquisher", "BIS")

LoonBestInSlot:AddItem(spec, "32593", "Feet", "Treads of the Den Mother", "BIS")
LoonBestInSlot:AddItem(spec, "33880", "Feet", "Vindicator's Dragonhide Boots", "BIS Mit")
LoonBestInSlot:AddItem(spec, "32366", "Feet", "Shadowmaster's Boots", "Alt")
LoonBestInSlot:AddItem(spec, "33582", "Feet", "Footwraps of Wild Encroachment", "Alt")
LoonBestInSlot:AddItem(spec, "33222", "Feet", "Nyn'jah's Tabi Boots", "Alt")

LoonBestInSlot:AddItem(spec, "33296", "Neck", "Brooch of Deftness", "BIS")
LoonBestInSlot:AddItem(spec, "24114", "Neck", "Braided Eternium Chain", "Alt")
LoonBestInSlot:AddItem(spec, "32362", "Neck", "Pendant of Titans", "Alt")
LoonBestInSlot:AddItem(spec, "33923", "Neck", "Vindicator's Pendant of Triumph", "Alt")
LoonBestInSlot:AddItem(spec, "30017", "Neck", "Telonicus's Pendant of Mayhem", "Alt")
LoonBestInSlot:AddItem(spec, "32405", "Neck", "Verdant Sphere", "Alt")
LoonBestInSlot:AddItem(spec, "32508", "Neck", "Necklace of the Deep", "Alt")

LoonBestInSlot:AddItem(spec, "29279", "Ring", "Violet Signet of the Great Protector", "BIS")
LoonBestInSlot:AddItem(spec, "32266", "Ring", "Ring of Deceitful Intent", "BIS")
LoonBestInSlot:AddItem(spec, "29301", "Ring", "Band of the Eternal Champion", "BIS")
LoonBestInSlot:AddItem(spec, "33496", "Ring", "Signet of Primal Wrath", "Alt")
LoonBestInSlot:AddItem(spec, "33919", "Ring", "Vindicator's Band of Triumph", "Alt Mit")

LoonBestInSlot:AddItem(spec, "32501", "Trinket", "Shadowmoon Insignia", "BIS Mit")
LoonBestInSlot:AddItem(spec, "34578", "Trinket", "Battlemaster's Determination", "BIS Mit")
LoonBestInSlot:AddItem(spec, "33831", "Trinket", "Berserker's Call", "BIS Thrt")
LoonBestInSlot:AddItem(spec, "32505", "Trinket", "Madness of the Betrayer", "BIS Thrt")
LoonBestInSlot:AddItem(spec, "28830", "Trinket", "Dragonspine Trophy", "BIS Thrt")
LoonBestInSlot:AddItem(spec, "32658", "Trinket", "Badge of Tenacity", "Alt Thrt")
LoonBestInSlot:AddItem(spec, "30627", "Trinket", "Tsunami Talisman", "Alt Thrt")
LoonBestInSlot:AddItem(spec, "28579", "Trinket", "Romulo's Poison Vial", "Alt Thrt")
LoonBestInSlot:AddItem(spec, "28288", "Trinket", "Abacus of Violent Odds", "Alt Thrt")

LoonBestInSlot:AddItem(spec, "30883", "2H", "Pillar of Ferocity", "BIS")
LoonBestInSlot:AddItem(spec, "33465", "2H", "Staff of Primal Fury", "Alt")

LoonBestInSlot:AddItem(spec, "33509", "Idol", "Idol of Terror", "BIS")
LoonBestInSlot:AddItem(spec, "32387", "Idol", "Idol of the Raven Goddess", "BIS")
LoonBestInSlot:AddItem(spec, "32257", "Idol", "Idol of the White Stag", "Alt")
